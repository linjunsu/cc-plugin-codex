...
const DEFAULT_MARKETPLACE_NAME = "";
const DEFAULT_MARKETPLACE_SOURCE = "";

function usage() {
  console.error("Usage: cc-plugin-codex <install|update|uninstall>");
  console.error("This fork installs through scripts/install-personal.mjs by default.");
  console.error("Set CC_PLUGIN_CODEX_MARKETPLACE_NAME and CC_PLUGIN_CODEX_MARKETPLACE_SOURCE to use a remote marketplace.");
  process.exit(1);
}
...
function resolveInstallerMarketplaceConfig() {
  ...
  if (!marketplaceName || !source) {
    throw new Error(
      "Remote marketplace install is not configured for this fork. Use `node scripts/install-personal.mjs`, or set CC_PLUGIN_CODEX_MARKETPLACE_NAME and CC_PLUGIN_CODEX_MARKETPLACE_SOURCE."
    );
  }
