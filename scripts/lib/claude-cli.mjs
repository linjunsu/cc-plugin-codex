...
const MAX_TOOL_LOG_FIELD_CHARS = 4000;
const MAX_TOOL_LOG_BODY_CHARS = 12000;
const SENSITIVE_KEY_RE = /(?:api[_-]?key|authorization|bearer|cookie|password|passwd|secret|session|token)/i;
...
function truncateForLog(value, maxChars = MAX_TOOL_LOG_FIELD_CHARS) {
...
function redactSensitiveText(value) {
  ...
  text = text.replace(
    /\b([A-Za-z0-9_.-]*(?:api[_-]?key|authorization|cookie|password|passwd|secret|session|token)[A-Za-z0-9_.-]*)\b\s*[:=]\s*(['"]?)([^'"\s,;&]+)/gi,
    "$1=$2[REDACTED]"
  );
  return text;
}
...
function sanitizeLogValue(value) {
  ...
  if (typeof value === "string") return truncateForLog(redactSensitiveText(value));
  return truncateForLog(redactSensitiveText(JSON.stringify(value, null, 2)));
}
  ...
  for (const [key, value] of Object.entries(input)) {
    if (SENSITIVE_KEY_RE.test(key)) {
      ...
      output[key] = value.map((item) =>
        item && typeof item === "object" ? sanitizeInputObject(item) : sanitizeLogValue(item)
      );
    } else if (value && typeof value === "object") {
      output[key] = sanitizeInputObject(value);
    } else {
      output[key] = value;
    }
  }
...
function toolFilePath(tool, input) {
...
function formatToolLogBody(tool, input) {
  const safe = sanitizeInputObject(input) ?? {};
  const lines = [];
  const command = toolCommand(safe);
  const filePath = toolFilePath(tool, safe);
  if (command) lines.push("command:", command);
  if (filePath) lines.push(`file: ${filePath}`);
  if (safe.description) lines.push(`description: ${safe.description}`);
  if (safe.pattern) lines.push(`pattern: ${safe.pattern}`);
  if (safe.glob) lines.push(`glob: ${safe.glob}`);
  if (safe.offset != null) lines.push(`offset: ${safe.offset}`);
  if (safe.limit != null) lines.push(`limit: ${safe.limit}`);

  if (tool === "Edit") {
    if (safe.old_string != null) lines.push("old_string:", sanitizeLogValue(safe.old_string));
    if (safe.new_string != null) lines.push("new_string:", sanitizeLogValue(safe.new_string));
    if (safe.replace_all != null) lines.push(`replace_all: ${safe.replace_all}`);
  } else if (tool === "MultiEdit" && Array.isArray(safe.edits)) {
    safe.edits.slice(0, 20).forEach((edit, index) => {
      lines.push(`edit ${index + 1}:`);
      if (edit?.old_string != null) lines.push("old_string:", sanitizeLogValue(edit.old_string));
      if (edit?.new_string != null) lines.push("new_string:", sanitizeLogValue(edit.new_string));
      if (edit?.replace_all != null) lines.push(`replace_all: ${edit.replace_all}`);
    });
    if (safe.edits.length > 20) lines.push(`[... ${safe.edits.length - 20} more edits omitted ...]`);
  } else if (tool === "Write" && safe.content != null) {
    lines.push("content:", sanitizeLogValue(safe.content));
  } else if (!command && !filePath) {
    lines.push(JSON.stringify(safe, null, 2));
  }

  return truncateForLog(lines.filter((line) => line !== "").join("\n"), MAX_TOOL_LOG_BODY_CHARS);
}
...
function summarizeToolUse(tool, input) {
  ...
  return {
    ...
    message: messageDetail
      ? `Using tool: ${tool} - ${truncateForLog(messageDetail, 180).replace(/\r?\n/g, " ")}`
      : `Using tool: ${tool}`,
    logTitle: `Tool input: ${tool}`,
    logBody: formatToolLogBody(tool, input)
  };
}

function contentBlockIndex(event) {
  return Number.isInteger(event?.event?.index) ? event.event.index : null;
}

function parseToolInputFromBlock(block) {
  if (!block) return {};
  const text = String(block.inputText ?? "").trim();
  if (!text) return block.input && typeof block.input === "object" ? block.input : {};
  try {
    ...
    try {
      ...
      return block.input && typeof block.input === "object"
        ? { ...block.input, _unparsed_input_json: text }
        : { _unparsed_input_json: text };
    }
  }
}
  ...
  _handleStreamEvent(event) {
    ...
    if (inner?.type === "content_block_delta") {
      ...
      if (blockDelta?.type === "input_json_delta") {
        ...
        const block = index == null ? null : this.toolBlocks.get(index);
        if (block) {
          block.inputText += blockDelta.partial_json ?? "";
        }
        return null;
      }
    }
      ...
      if (cb?.type === "tool_use") {
        ...
        if (index != null) {
          this.toolBlocks.set(index, {
            tool: cb.name,
            input: cb.input && typeof cb.input === "object" ? cb.input : {},
            inputText: ""
          });
        }
        return {
    ...
    if (inner?.type === "content_block_stop") {
      ...
      if (block) {
        ...
        const input = parseToolInputFromBlock(block);
        const toolUse = summarizeToolUse(block.tool, input);
        pushBoundedTail(
          this.state.toolUses,
          {
            tool: toolUse.tool,
